...
namespace gstd {
    ...
    auto VirtualMemorySource::DeallocateRegion(const Span<Byte> &region) -> void {
        // TODO: check
...
int main() {
    {
        auto memorySource = gstd::VirtualMemorySource::New();
        auto region = memorySource.AllocateRegion(2 * gstd::Kb);
        auto allocator = gstd::FreeListAllocator::New(region);
        auto ptr = allocator.Allocate<int>();
        *ptr = 12;
    }
